...
public class OpenHashMapEdges extends
	...
	public EdgeValue getEdgeValue(LongWritable targetVertexId) {
		/*
		 * Adnan :  get(key) returns the corresponding value, 
		 * or the default return value if no value was present for the given key.
		 * For Long2ShortMap this value equal to 0, so this will return an EdgeValue 
		 * Object with a partition set to 0 which is not correct.
		 */
		short v = map.get(targetVertexId.get());
		/*
		 * If we return null to say that no value exists, this will cause problem too 
		 * because the partitioning algorithm could use 0 as partition id
		 */
		//if (v==map.defaultReturnValue()) return null;

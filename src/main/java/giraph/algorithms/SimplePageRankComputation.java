...
public class SimplePageRankComputation
		extends BasicComputation<BGRAP_LongWritable, DoubleWritable, FloatWritable, DoubleWritable> {
	...
	@Override
	public void compute(Vertex<BGRAP_LongWritable, DoubleWritable, FloatWritable> vertex,
	...
	public static class SimplePageRankWorkerContext extends WorkerContext {
		...
		public void preSuperstep() {
			if (getSuperstep() >= 3) {
				LOG.info("aggregatedNumVertices=" + getAggregatedValue(SUM_AGG) + " NumVertices="
						+ getTotalNumVertices());
				if (this.<LongWritable>getAggregatedValue(SUM_AGG).get() != getTotalNumVertices()) {
					throw new RuntimeException("wrong value of SumAggreg: " + getAggregatedValue(SUM_AGG)
							+ ", should be: " + getTotalNumVertices());
				}
	...
	public static class SimplePageRankVertexReader
			extends GeneratedVertexReader<BGRAP_LongWritable, DoubleWritable, FloatWritable> {
		/** Class logger */
		...
		@Override
		public Vertex<BGRAP_LongWritable, DoubleWritable, FloatWritable> getCurrentVertex() throws IOException {
			Vertex<BGRAP_LongWritable, DoubleWritable, FloatWritable> vertex = getConf().createVertex();
			BGRAP_LongWritable vertexId = new BGRAP_LongWritable(
					(inputSplit.getSplitIndex() * totalRecords) + recordsRead);
			vertexId.setPartition( (short) (vertexId.getId() % getConf().getMaxWorkers()));
			DoubleWritable vertexValue = new DoubleWritable(vertexId.getId() * 10d);
			long targetVertexId = (vertexId.getId() + 1) % (inputSplit.getNumSplits() * totalRecords);
			float edgeValue = vertexId.getId() * 100f;
			List<Edge<BGRAP_LongWritable, FloatWritable>> edges = Lists.newLinkedList();
			edges.add(EdgeFactory.create(new BGRAP_LongWritable(targetVertexId, 
					(short) (targetVertexId % getConf().getMaxWorkers())),
					new FloatWritable(edgeValue)));
			vertex.initialize(vertexId, vertexValue, edges);
			++recordsRead;
			if (LOG.isInfoEnabled()) {
				LOG.info("next: Return vertexId=" + vertex.getId().getId() + ", vertexValue=" + vertex.getValue()
	...
	public static class SimplePageRankVertexInputFormat
			extends GeneratedVertexInputFormat<BGRAP_LongWritable, DoubleWritable, FloatWritable> {
		@Override
		public VertexReader<BGRAP_LongWritable, DoubleWritable, FloatWritable> createVertexReader(InputSplit split,
	...
	public static class SimplePageRankVertexOutputFormat
			extends TextVertexOutputFormat<BGRAP_LongWritable, DoubleWritable, FloatWritable> {
		@Override
		...
		public class SimplePageRankVertexWriter extends TextVertexWriter {
			@Override
			public void writeVertex(Vertex<BGRAP_LongWritable, DoubleWritable, FloatWritable> vertex)
					throws IOException, InterruptedException {

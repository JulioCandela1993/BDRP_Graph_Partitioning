 ...
 * new information, X can decide to connect to vertices (people) in the result
 * array or not.
 *
...
public class SimpleTriangleClosingComputation extends
		BasicComputation<BGRAP_LongWritable, SimpleTriangleClosingComputation.BGRAP_LongWritableArrayListWritable, DoubleWritable, BGRAP_LongWritable> {
	/** Vertices to close the triangle, ranked by frequency of in-msgs */
	private Map<BGRAP_LongWritable, Integer> closeMap = Maps.<BGRAP_LongWritable, Integer>newHashMap();

	@Override
	public void compute(Vertex<BGRAP_LongWritable, BGRAP_LongWritableArrayListWritable, DoubleWritable> vertex, Iterable<BGRAP_LongWritable> messages)
			throws IOException {
		if (getSuperstep() == 0) {
			...
			for (BGRAP_LongWritable message : messages) {
				// count in-message frequency 
				final int current = (closeMap.get(message) == null) ? 0 : closeMap.get(message) + 1;
			...
			for (Map.Entry<BGRAP_LongWritable, Integer> entry : closeMap.entrySet()) {
				sortedResults.add(new Pair(entry.getKey(), entry.getValue()));
			}
			BGRAP_LongWritableArrayListWritable outputList = new BGRAP_LongWritableArrayListWritable();
	...
	public static class Pair implements Comparable<Pair> {
		 ...
		 */
		private final BGRAP_LongWritable key;
		/**
		 * value
		 * 
		 ...
		 * @param v the value
		 */
		public Pair(BGRAP_LongWritable k, Integer v) {
			key = k;
			value = v;
		}
		 ...
		 * @return the key
		 */
		public BGRAP_LongWritable getKey() {
			return key;
		}
	...
	/**
	 * Utility class for delivering the array of vertices THIS vertex should connect
	 * with to close triangles with neighbors
	 */
	@SuppressWarnings("serial")
	public static class BGRAP_LongWritableArrayListWritable extends ArrayListWritable<BGRAP_LongWritable> {
		/** Default constructor for reflection */
		public BGRAP_LongWritableArrayListWritable() {
			super();
		}
		...
		public void setClass() {
			setClass(BGRAP_LongWritable.class);
		}
	}
}
